...
public class Table {
    ...
    public Table(Env env, Integer[] slotToCard, Integer[] cardToSlot) {
        ...
        playersSlots = new mySlots[env.config.players];
        for (int i = 0; i < playersSlots.length; i++) {
            playersSlots[i] = new mySlots();
        }
    }
    ...
    public void placeToken(int player, int slot) {
        if (slotToCard[slot] != null) {
            ...
            playersSlots[player].addSlot(slot);
        }
    }
    ...
    public boolean removeToken(int player, int slot) {
        if (slot < env.config.columns * env.config.rows && slotToCard[slot] != null) { // beacuse of the warnings,
                                                                                       // alwase true
            env.ui.removeToken(player, slot);
            return playersSlots[player].deleteSlot(slot);
        }
        return false;
    }

    /////////////////////////////////////////////////////
    public void removeCardsFromTable(int[] slots) {
